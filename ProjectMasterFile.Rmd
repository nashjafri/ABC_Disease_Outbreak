...
In the following code, we define a function that takes parameters $q_c$, $q_h$ and (maximum) household size, and returns the probability distribution matrix `w_js_matrix` with entries $w_{js}$ for $j = 0,1,\dots, s$, $s = 1 ,2, \dots,$household_size. This is done iteratively based on the formula $w_{js} = \binom{s}{j}w_{jj}(q_cq_h^j)^{s-j}$, $w_{0s} = q_c^s$ and $w_{ss} = 1 - \sum_{j = 0}^{s-1}w_{js}$.
...
Next, we generate posterior samples by defining a function that takes the pair of observed data ($\text{given_data1}$, $\text{given_data2}$), a similarity measure ($\text{distance}$), a prior distribution for the parameter, a data-simulating function, and a tolerance level. This function returns a posterior sample of the parameter $q$ (in our case, the 4-parameter vector $(q_{c1}, q_{h1}, q_{c2}, q_{h2})$) that satisfies the tolerance condition for the distance between simulated and given data.
...
Since both observed data and simulated data are 6×5 matrices with integer entries, and 10 of the entries are always zero, the Frobenius distance can be sensitive to small differences. For instance, if all of the remaining 20 entries are identical except one, the Frobenius distance is 1. Similarly, if 5 of the 20 entries are identical and remaining 15 differ by only 1, the Frobenius distance becomes 15. This scale justifies setting $\epsilon = 15$ as a reasonable threshold, which is what we use to generate the posteriors.
...
In general, we noted that it takes about 20-30 seconds in RStudio to generate one posterior sample with $\epsilon = 15$. Whereas it takes about 49-60 minutes in Google Colab (using T4 GPU) to generate one with $\epsilon = 10$.
...
- given_data3: The first dataset represents Influenza B infection in 1975-76, Seattle, Washington. [4]
- given_data4: The second dataset represents Influenza A (H1N1) infection in 1978-79, Seattle, Washington. [4]  
...
$\epsilon = 6$ takes about 15-30 seconds per simulation. It would not be computationally feasible to draw 100 samples satisfying such a strict acceptance condition.

```{r}
system.time(generate_abc_sample(given_data3, given_data4, distance, 
                    prior_distribution, simulate_household_data, epsilon = 6)
)
```

The code below produces 100 parameter samples that are accepted by the algorithm with $\epsilon = 7$.
...
# Generating 100 posterior samples
posterior_samples2 <- replicate(n = 100,
    generate_abc_sample(given_data3, given_data4, distance, 
                        prior_distribution, simulate_household_data, epsilon = 7))
```
...
```{r, cache=TRUE}
# Recreate Figure 3(b) with n = 100 posterior samples and epsilon = 8

par(pty = "s")      

plot(q_h3, q_c3, 
     main = "q_h vs q_c (n = 100, epsilon = 7)", 
     xlab = "q_h", ylab = "q_c", 
     xlim = c(0, 1), ylim = c(0, 1), 
     pch = 16, col = "red", 
     cex = 1.5)  
