...
Now with these modified functions we are able to build a 2 parameter model for influenza A. To do this we will first combine the data given for the 1977-78 and 1980-81 outbreaks of influenza A into one matrix by adding the matrices for the separate outbreaks. Another approach could be to still use the average Frobenius distance between the separate datasets and two simulated ones, which perhap the authors do.
...
Below we write a function to compare the 2-parameter model (model 1) and the 4-parameter model (model 2) for the two outbreaks of influenza A strain. Prior over models is uniform over model 1 and model 2, prior for parameters is Uniform[0,1] for both models and data generating function is the same as before, i.e. `simulate_household_data`. Here we take the average Frobenius distance for the 2-parameter model to replicate the authors' results in Figure 1(a) of Supplementary data. 
...
calculate_model_postr <- function(given_data1, given_data2, epsilon, n_samples) {
  ...
  for (i in 1:n_samples) {
    ...
    if (distance(given_data1, given_data2, simulated_data11, simulated_data12) < epsilon) {
    ...
    if (distance(given_data1, given_data2, simulated_data21, simulated_data22) < epsilon) {
...
model_postr_100 <- calculate_model_postr(given_data1, given_data2, epsilon = 100, n_samples = 1000)

model_postr_80 <- calculate_model_postr(given_data1, given_data2, epsilon = 80, n_samples = 1000)

model_postr_50 <- calculate_model_postr(given_data1, given_data2, epsilon = 50, n_samples = 1000)

model_postr_30 <- calculate_model_postr(given_data1, given_data2, epsilon = 30, n_samples = 1000)

model_postr_20 <- calculate_model_postr(given_data1, given_data2, epsilon = 20, n_samples = 1000)
```
...
Similarly, we write a function to compare the 3-parameter model (model 1) and the 4-parameter model (model 2) for the outbreaks of influenza A and influenza B strains. This section replicates the findings of Figure 3(b) from the Supplementary data.
...
calculate_model_postr2 <- function(given_data1, given_data2, epsilon, n_samples) {
...
Model_postr_40 <- calculate_model_postr2(given_data3, given_data4, epsilon = 40, n_samples = 1000)

Model_postr_20 <- calculate_model_postr2(given_data3, given_data4, epsilon = 20, n_samples = 1000)

Model_postr_15 <- calculate_model_postr2(given_data3, given_data4, epsilon = 15, n_samples = 1000)

Model_postr_10 <- calculate_model_postr2(given_data3, given_data4, epsilon = 10, n_samples = 1000)

Model_postr_8 <- calculate_model_postr2(given_data3, given_data4, epsilon = 8, n_samples = 1000)
```
...
Here we replicate some of the results proposed by Toni and Stumpf. In particular, we are able to reproduce figure 3. The 4-parameter model proposed in [1] gives a good way of telling whether the ABC procedure gives the same parameters for modeling two different outbreaks. From this, we conclude that outbreaks of the same strain of influenza are best modeled by a 2-parameter model and that outbreaks from different strains of influenza are best modeled by a 3-parameter model which accounts for differences in $q_c$ while leaving $q_h$ constant.
...
### Tests for two parameter distance

```{r}
# Tests for distance_2
...
test_that("distance_2 throws error for bad parameters", {
  ...
  expect_error(distance_2(size1, size2))   # Given_data and simulated_data have different sizes
})

test_that("distance works on simple cases: distance between given data and simulated data is 0 when given and simulated data are identical",{
  data_1 <- matrix(c(1,1,1,1), nrow=2)
  
  expect_equivalent(distance_2(data_1, data_1), 0)
})
...
test_that("distance works on simple cases",{
  ...
  expect_equivalent(distance_2(given_data, simulated_data), 2)
})
...
test_that("generate_abc_sample throws errors for bad parameters", {
  ...
  expect_error(generate_abc_sample_2(given_data1, distance_2, 
...
test_that("generate_abc_sample throws errors for bad parameters", {
  ...
  expect_error(generate_abc_sample_3(given_data1, given_data2, distance, 
...
**Note:** Tests for the functions `calculate_model_postr` and `calculate_model_postr2` are not included but can be performed in a similar manner as the tests for `generate_abc_sample` functions.
